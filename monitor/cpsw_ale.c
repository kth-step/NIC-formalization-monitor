...
//	__raw_writel(idx, ale->params.ale_regs + ALE_TABLE_CONTROL);
	monitor__raw_writel(idx, ale->params.ale_regs + ALE_TABLE_CONTROL);
...
static int cpsw_ale_write(struct cpsw_ale *ale, int idx, u32 *ale_entry)
{
	...
	for (i = 0; i < ALE_ENTRY_WORDS; i++)
//		__raw_writel(ale_entry[i], ale->params.ale_regs + ALE_TABLE + 4 * i);
		monitor__raw_writel(ale_entry[i], ale->params.ale_regs + ALE_TABLE + 4 * i);

//	__raw_writel(idx | ALE_TABLE_WRITE, ale->params.ale_regs + ALE_TABLE_CONTROL);
	monitor__raw_writel(idx | ALE_TABLE_WRITE, ale->params.ale_regs + ALE_TABLE_CONTROL);
...
//	__raw_writel(tmp, ale->params.ale_regs + offset);
	monitor__raw_writel(tmp, ale->params.ale_regs + offset);
