...
#define dma_reg_write(ctlr, ofs, v)	monitor__raw_writel(v, (ctlr)->dmaregs + (ofs))
#define chan_write(chan, fld, v)	monitor__raw_writel(v, (chan)->fld)
#define desc_write(desc, fld, v)	monitor__raw_writel((u32)(v), &(desc)->fld)
				...
				int size, int align)
{
	...
	if (phys) {
		...
		monitor_set_cppi_ram_base_va(pool->iomap);
//		printk(KERN_INFO "Virtual address of CPPI_RAM: 0x%p\n", pool->iomap);
//		printk(KERN_INFO "Physical address of CPPI_RAM: 0x%X\n", phys);
//		printk(KERN_INFO "Size of CPPI_RAM: 0x%X (%u)\n", size, size);
...
//		__raw_writel(0, ctlr->params.rxcp + 4 * i);
		monitor__raw_writel(0, ctlr->params.txhdp + 4 * i);
		monitor__raw_writel(0, ctlr->params.rxhdp + 4 * i);
		monitor__raw_writel(0, ctlr->params.txcp + 4 * i);
		monitor__raw_writel(0, ctlr->params.rxcp + 4 * i);
	}
...
static int __cpdma_chan_process(struct cpdma_chan *chan)
{
	...
	if ((status & (CPDMA_DESC_EOQ | CPDMA_DESC_TD_COMPLETE)) == CPDMA_DESC_EOQ) {
		chan->stats.requeue++;
